...
import { create, findAll, findOne, remove, restore, update } from '../components/sale-component';
...
describe('VENTAS-ACCIONES', () => {
  describe('SUCCESS CASES (200)', () => {
    test.each([
      ...
      expect(res.json).toHaveBeenCalledWith({
        ...
        message_reorder: expect.any(String),
        content: expect.any(Object)
      });
    });

    it('Obtener ventas', async () => {
      const req = { user: { id: 1 } };
    ...
    test.each([
      [{ params: {id:3} , data: { product_id:1, quantity:2 }, userId: 1 }],
      [{ params: {id:4} , data: { product_id:2, quantity:2  }, userId: 1 }],
    ])('Actualizar venta: %o', async (options) => {
    ...
    ])('Eliminar venta: %o', async (options) => {
      const req = { params:options.params, user: { id: options.userId } };
      const res = { json: jest.fn(), status: jest.fn(() => res) };
      const next = jest.fn();
  ...
  describe('ERROR CASES (400)', () => {
    it('(Error):Crear venta (DatosRequeridos)', async () => {
    ...
    ])('(Error):Obtener venta (IdNoExiste): %o', async (options) => {
      const req = { params:options.params, user: { id: options.userId } };
      const res = { json: jest.fn(), status: jest.fn(() => res) };
      const next = jest.fn();
      await findOne(req as any, res as any, next as any);
    ...
    test.each([
      [{ params: {id:1001} , data: { product_id:1, quantity:2  }, userId: 1 }],
      [{ params: {id:1002} , data: { product_id:2, quantity:2  }, userId: 1 }],
    ])('(Error):Actualizar venta (IdNoExiste):', async (options) => {
      const req = { params:options.params,body:options.data, user: { id: options.userId } };
      const res = { json: jest.fn(), status: jest.fn(() => res) };
      const next = jest.fn();
      await update(req as any, res as any, next as any);
    ...
    ])('(Error):Eliminar venta (IdNoExiste): %o', async (options) => {
      const req = { params:options.params, user: { id: options.userId } };
      const res = { json: jest.fn(), status: jest.fn(() => res) };
      const next = jest.fn();
      await remove(req as any, res as any, next as any);
